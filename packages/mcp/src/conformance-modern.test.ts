...
describe('MCP 2026-07-28 (modern) conformance', () => {
	...
	it('reads a resource and gets a prompt on the modern era (every advertised primitive kind, not just tools)', async () => {
		...
		expect(resources.resources.some((resource) => resource.name === 'user_profile')).toBe(true);

		const resourceResult = await client.readResource({ uri: 'user://profile' });
		...
		const promptResult = await client.getPrompt({
			name: 'summarize',
			arguments: { topic: 'model context protocol' },
		});
	...
	// PROTO-001's verifier flagged this test as a doubt worth resolving under
	// PROTO-002: the request below carries no `mcp-protocol-version` header
	// and no per-request `_meta` envelope claim, and the SDK's own
	// `isLegacyRequest` documentation is explicit that a claim-less
	// notification POST classifies as LEGACY traffic (served through the
	// stateless fallback), not the modern path — confirmed directly, not
	// assumed: this exact request reaches `serveLegacyRoute`, not
	// `serveModern`, in the installed SDK. Renamed to say what it actually
	// proves; the genuine modern-lane case follows immediately after.

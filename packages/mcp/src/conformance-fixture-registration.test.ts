 ...
 * (`conformance-fixture-registration.ts` lines 254-263, 314-320, 371-377,
 * 429-435) are dead code against the installed
 * `@modelcontextprotocol/server@2.0.0` SDK, confirmed by reading its
 * `dist/src-CX2iR2pK.mjs`: every per-request `ctx.mcpReq` this SDK builds
 * (see the `Protocol`-level request dispatcher, ~line 6420) always attaches
 * a `send` function, unconditionally, regardless of era or the client's
 * declared capabilities -- `readRequestSender(ctx)` (`handler-context.ts`)
 * can therefore never observe it missing through any real client/server
...
describe('conformance fixture tools: simple content shapes', () => {
	...
	it('test_multiple_content_types returns text, image, and resource blocks together', async () => {
		...
		expect((result.content?.[2] as { type?: string }).type).toBe('resource');
		await client.close();
	});
});
...
describe('conformance fixture tools: sampling', () => {
	it('test_sampling relays the client sampling response as LLM response text', async () => {
		...
		expect(result.isError).not.toBe(true);
		expect((result.content?.[0] as { text?: string }).text).toBe('LLM response: a sampled reply');

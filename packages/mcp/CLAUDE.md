...
- `src/env.ts` — Owns `MCP_SERVER_NAME`, `MCP_CONFORMANCE_MODE`, `LOG_LEVEL`, `NODE_ENV`, and `LOG_CONTENT_DIAGNOSTICS_UNTIL` (refused outright in production — see `OBS-001`'s note in `RUNBOOK.md`). The actual Zod shape lives in `src/environment-schema.ts`; `env.ts` is a thin `environmentalist.sync({...})` wrapper plus the `SKIP_ENV_VALIDATION` rejection (`CONFIG-001` — that variable does not exist in this codebase under any name; setting it throws immediately). Consume it as `environment.mcpServerName`, `environment.nodeEnv`, etc. — Environmentalist camel-cases every schema key.

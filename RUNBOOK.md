...
| --------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Client registration                           | `event: "oauth_client_registration"`, `outcome: "success"`                                                                                                            | `applications/web/src/routes/oauth-routes.ts`                                          | Logs the issued `clientId`, never the secret.                                                                                                                                                                                           |
| User denial                                   | `event: "oauth_authorization"`, `outcome: "user_denied"`                                                                                                              | `oauth-routes.ts` (deny handler)                                                       |                                                                                                                                                                                                                                         |
| Invalid client                                | `event: "oauth_client_authentication"`, `outcome: "invalid_client"`                                                                                                   | `oauth-routes.ts` (`authenticateOauthClient`, shared by token exchange and revocation) | Logs the caller-presented `clientId`, never the credential it failed with.                                                                                                                                                              |
| Invalid resource (RFC 8707 audience mismatch) | `event: "oauth_token_exchange"`, `outcome: "invalid_resource"` (token/refresh grants); `event: "mcp_authentication"`, `outcome: "invalid_resource"` (`/mcp` boundary) | `oauth-routes.ts`, `applications/web/src/routes/mcp-routes.ts`                         | The `/mcp` boundary case is deliberately identical on the wire to "expired or invalid token" (RFC 6750 §3.1 — collapsing them prevents a caller probing which reason applies); the log line is where they are actually distinguishable. |
